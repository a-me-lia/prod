...
<div className='flex flex-col items-center  h-[calc(100dvh)]  xl:min-h-screen overflow-hidden bg-slate-700 text-white'>
  ...
  <h1 className='text-2xl mt-16 mx:mt-2 xl:text-4xl'>Names for pickup:</h1>
  <p className='mx-8 text-center mt-4 mb-2'>click anywhere on the page to enable audio ping!</p>   
    ...
    <div className='items-center flex flex-col'>        
      ...
      <ul className="flex flex-col items-center">
        {entries.map((entry, index) => (
          <li className='flex flex-col text-2xl mt-6 mb-4 xl:w-full xl:mt-12 xl:mb-8 xl:text-6xl font-bold text-center text-white' key={index}>
            <span>{entry.username}</span>
            <span className="mt-2 font-normal text-sm xl:mt-4 xl:text-4xl">Timestamp: {entry.timestamp}</span>
